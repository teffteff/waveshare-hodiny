...
| Učení pro Adama (matematika, čeština) | 8104 + 8105, jen loopback | `/opt/learning` (kód), `/var/lib/learning/progress.json` (postup), `learning-web.service`; procvičování `https://$FLEET_DOMAIN/uceni/` (heslo řeší služba, zařízení si ho pamatuje; `/adam` přesměrovává), statistiky `https://$FLEET_DOMAIN/hodiny/adam/` za heslem novinek | vlastní repozitář `learning` (`~/nasobilka`) |
...
proto klíče po projektech, tři projekty v AI Studiu (ověřeno 24. 9. 2026):
`GEMINI_KEY_HODINY` (zprávy), `GEMINI_KEY_WATCH` (hlídač obchodů) a
