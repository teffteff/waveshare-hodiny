...
void testIncompleteOrDuplicateScreenOrderIsRejected() {
  ...
  // Pět jmen bylo úplné pořadí do schématu 36, šest do schématu 41; s obrazovkou
  // Slunce a Měsíce už chybí jedno.
  assert(!parseScreenOrder("clock,radar,rss,forecast,planes", order));
  assert(!parseScreenOrder("clock,radar,rss,forecast,planes,agenda", order));
