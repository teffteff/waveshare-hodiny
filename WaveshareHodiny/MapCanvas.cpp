...
bool mapPlaceIconLabel(MapLabelPlacer &placer, int x, int y, int textWidth,
  ...
  const MapLabelBox candidates[] = {
      {x - width / 2, y + VERTICAL_GAP, width, height},
      {x - width / 2, y - VERTICAL_GAP - height, width, height},
      {x + HORIZONTAL_GAP, y - height / 2, width, height},
      {x - HORIZONTAL_GAP - width, y - height / 2, width, height},
  };
